...
INCLUDES := ./include ./deps $(wildcard ./deps/*/include)
...
#DIALYZER_APPS = kernel stdlib sasl erts ssl tools os_mon runtime_tools \
				...
				common_test syntax_tools compiler

DIALYZER_APPS = kernel stdlib sasl erts ssl crypto inets xmerl public_key
...
ct:
	@echo Sorry, tests will not work because valid apns certs
	@echo and tokens are needed, or we need to write a
	@echo simulator.
	#$(REBAR) skip_deps=true ct

test: compile ct

dialyzer: deps $(DEPS_PLT)
	dialyzer \
		--fullpath \
		--plt $(DEPS_PLT) \
		$(DIALYZER_WARNINGS) \
...
clean-doc-data:
	...
	- rm -f $(patsubst ./src/%.erl, $(CURDIR)/doc/%.md, $(SOURCES))
